...
function getNames(people) {
  ...
  people.forEach((person) =>
    newPeople.push(person.firstName + " " + person.lastName)
  );
  return newPeople;
}
...
function translateToPirateTalk(phrase) {
  ...
  return arrWords
    .map((word) => {
      return EN_PIRATE_LOOKUP[`${word}`] ? EN_PIRATE_LOOKUP[`${word}`] : word;
    })
    .join(" "); // Creates a string by concatenating all of the elements in the mapped array
}
...
function wordCount(str) {
  ...
  str.split(" ").forEach((word) => {
    const count = countOfWords[`${word}`] ? countOfWords[`${word}`] + 1 : 1;
    countOfWords[`${word}`] = count;
  });
  return countOfWords;
}
...
function buildBugHuntCalendar(bugs) {
  const calendar = {};
  for (let i = 1; i <= 12; i++) calendar[`${i}`] = []; // Populate calendar with 12 months
  for (const bug in bugs) {
    bugs[`${bug}`].availability.months.forEach((month) =>
      calendar[`${month}`].push(bugs[`${bug}`].name)
    );
  }
  return calendar;
}
